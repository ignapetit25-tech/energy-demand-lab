...
The report now includes an independently sourced CAMMESA branch snapshot for August 2026 and a separately dated INDEC capacity-utilization observation. A new [AI infrastructure register](https://ignapetit25-tech.github.io/energy-demand-lab/infrastructure.html) separates Argentine projects from international references, announced MW from measured GWh, and litigation from electricity evidence. These additions appear on the web and in the text report, with separate JSON downloads; they do not change the national-series CSV or Excel. [Scope and update procedure](docs/infraestructura-y-sectores.md).
