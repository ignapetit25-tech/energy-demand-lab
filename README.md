...
The [sector review dashboard](https://ignapetit25-tech.github.io/energy-demand-lab/sector-alerts.html) applies the preserved proposal to 44 comparable complete months and exposes 27 sensitivity combinations plus four one-factor checks. August has six baseline priorities, versus four to eight across the 27 combinations. These are descriptive review signals, not statistically calibrated alarms. The physical-production comparison matches July electricity with July INDEC IPI categories; August production remains missing. [Evidence and limitations](reports/research/produccion-y-sensibilidad.md). Rebuild with `python3 scripts/build_sector_alerts.py`; no notifications or data refresh are automated.

Research follow-up: [unsent Aluar/CAMMESA data requests](docs/solicitudes-datos-aluar-cammesa.md), [PDF-versus-daily-source reconciliation](reports/research/discrepancias-cammesa.md), and [original proposed sector review alerts](reports/research/alertas-sectoriales.md). The original proposal metadata is preserved; the new dashboard records its descriptive deployment separately. No causal finding or request submission is implied. Reproduce the original research with `python3 scripts/analyze_sector_followups.py` (requires Poppler `pdftotext`).

The September 26 extension reconstructs **56 complete months for 14 electricity activities** from 1,727 daily CAMMESA records. It adds monthly Aluar net-grid energy, explicitly missing gross-consumption/self-generation fields, and a three-sheet Excel extension. Earlier national worksheets and frozen forecasts are preserved. [Monthly-balance findings, source discrepancies and workbook checks](docs/balance-mensual-y-actividades.md).
...
The [sector-history explorer](https://ignapetit25-tech.github.io/energy-demand-lab/sector-history.html) incorporates 12 CAMMESA editions, 164 same-calendar-month historical records across four branches, and 19 months of provisional INDEC capacity utilization for twelve activities. Source coverage changes and missing periods are explicit, not silently spliced. The [AI infrastructure register](https://ignapetit25-tech.github.io/energy-demand-lab/infrastructure.html) documents five Argentine projects/facilities plus two US references, distinguishing technical capacity from measured energy and litigation from consumption evidence. The August text report includes the expanded diagnosis; separate JSON downloads preserve full provenance. The national CSV and original Excel sheets are preserved; the September 26 extension adds separate research sheets. [Scope and update procedure](docs/infraestructura-y-sectores.md).

Open [the dashboard](dashboard/index.html) or [the monthly sector report](dashboard/monthly-report.html) locally in a browser. The report describes year-on-year demand changes, sector contributions and like-for-like year-to-date totals. It supports month selection, plain-text and enriched Spanish CSV downloads, and browser printing. Its first report covers August 2026 from the September 20 source snapshot; [the reusable written report](reports/monthly/2026-08.md) includes evidence and limitations. It is descriptive, not climate-adjusted or causal. [AI/electricity methodology](docs/ia-electricidad.md) explains the sourced international evidence, the new Excel evidence sheet and why these Argentine aggregates cannot identify AI consumption.
